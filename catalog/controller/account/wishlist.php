	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			if ($product_info) {
				...
				if($users->num_rows == 0 || $users->row['customer_group_id']){
							$data['userInf'] = 'null';
					...
					$data['userInf'] = $users->row['customer_group_id'];
							if ($data['userInf'] == 3) {
								if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
									$price = $this->currency->format($this->tax->calculate($product_info['price2'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
								} else {
									$price = false;
								}
