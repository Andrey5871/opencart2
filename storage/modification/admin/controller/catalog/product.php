...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'name'       => $result['name'],
				'model'      => $result['model'],
				'price2'      => $this->currency->format($result['price2'],
	...
	protected function getForm() {
		...
		if (isset($this->error['sku'])) {
			$data['error_sku'] = "Ошибка, вы не ввели артикул продукта!";
		} else {
			$data['error_sku'] = '';
		}
		...
		if (isset($this->request->post['polarity'])) {
			$data['polarity'] = $this->request->post['polarity'];
		} elseif (!empty($product_info)) {
			$data['polarity'] = $product_info['polyaryti'];
		} else {
			$data['polarity'] = '';
		}

		if (isset($this->request->post['typeAkum'])) {
			$data['typeAkum'] = $this->request->post['typeAkum'];
		} elseif (!empty($product_info)) {
			$data['typeAkum'] = $product_info['typeAkum'];
		} else {
			$data['typeAkum'] = '';
		}

		if (isset($this->request->post['tokProkrutki'])) {
			$data['tokProkrutki'] = $this->request->post['tokProkrutki'];
		} elseif (!empty($product_info)) {
			$data['tokProkrutki'] = $product_info['tokProkrutki'];
		} else {
			$data['tokProkrutki'] = '';
		}

		if (isset($this->request->post['nomEmkost'])) {
			$data['nomEmkost'] = $this->request->post['nomEmkost'];
		} elseif (!empty($product_info)) {
			$data['nomEmkost'] = $product_info['emkostNomin'];
		} else {
			$data['nomEmkost'] = '';
		}


		if (isset($this->request->post['banki'])) {
			$data['banki'] = $this->request->post['banki'];
		} elseif (!empty($product_info)) {
			$data['banki'] = $product_info['countBanki'];
		} else {
			$data['banki'] = '';
		}
		...
		if (isset($this->request->post['priceOpt'])) {
			$data['priceOpt'] = $this->request->post['priceOpt'];
		} elseif (!empty($product_info)) {
			$data['priceOpt'] = $product_info['price2'];
		} else {
			$data['priceOpt'] = '';
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['sku']) < 1)) {
			$this->error['sku'] = "Вы не ввели артикул продукта";
		}
